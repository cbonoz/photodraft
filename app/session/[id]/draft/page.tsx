...
import { useSession, usePickPhoto, useResetDraft } from "@/lib/hooks";
    ...
    return (
      <main className="max-w-6xl mx-auto p-8">
        <div className="text-center mb-10">
          ...
          <p className="text-green-400 text-2xl font-semibold mb-2">Draft Complete!</p>
          <p className="text-neutral-400 mb-6">All {allPhotos.length} photos have been picked.</p>
              ...
              <div key={player.id} className="bg-neutral-900 rounded-xl p-4">
                <h3 className="font-semibold mb-2 flex items-center gap-2">
                  ...
                  <span className="text-neutral-500 text-sm ml-auto">
                    {playerPicks.length} photo{playerPicks.length !== 1 && "s"}
                  </span>
                </h3>
                <div className="grid grid-cols-2 gap-2">
                  {playerPicks.map((photo) => (
                    <div key={photo.id} className="aspect-square rounded-lg overflow-hidden bg-neutral-800">
        ...
        {players.map((player) => {
          ...
          return (
            ...
            >
              <h3 className="font-semibold mb-2 flex items-center gap-2">
                {player.name}
                {isCurrent && <span className="text-xs text-blue-400">Picking...</span>}
                <span className="text-neutral-500 text-sm ml-auto">
                  {playerPicks.length}
                </span>
              </h3>
              <div className="grid grid-cols-2 gap-2 min-h-[100px]">
                ...
                {playerPicks.length === 0 && (
                  <p className="text-neutral-600 text-sm col-span-2">No picks yet</p>
                )}
              </div>
            </div>
          );
