...
export default function AdminPage() {
  ...
  async function onUpload(e: FormEvent<HTMLFormElement>) {
    ...
    await uploadMutation.mutateAsync(Array.from(fileRef.current.files));
  ...
  return (
    <main className="max-w-4xl mx-auto p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        {draftComplete ? (
          <span className="px-3 py-1 rounded-full bg-green-900 text-green-300 text-sm">Complete</span>
        ) : draftActive ? (
          <span className="px-3 py-1 rounded-full bg-yellow-900 text-yellow-300 text-sm">In progress</span>
        ) : (
          <span className="px-3 py-1 rounded-full bg-neutral-800 text-neutral-400 text-sm">Setup</span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-neutral-900 rounded-xl p-6">
          ...
          <form onSubmit={onUpload} className="flex gap-2 mb-4">
            ...
            <button
              type="submit"
              disabled={uploadMutation.isPending || draftActive}
      ...
      {!session.closed && photos.length > 0 && players.length > 0 && (
        <div className="flex justify-center">
          ...
          >
            {startMutation.isPending ? "..." : "Start Draft"}
          </button>
        </div>
      )}
